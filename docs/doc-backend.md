...
- `ItauInvoiceParser` (reescrito na issue #73, **column-aware**): a fatura real do Itaú usa leiaute de **duas colunas**, que o `PDFTextStripper` mescla numa mesma linha física. O parser:
  ...
  - **Escopo dos lançamentos:** considera de `Lançamentos: compras e saques` até `Total dos lançamentos atuais`, incluindo nacionais e internacionais; a tabela `Compras parceladas - próximas faturas` (projeções) é **excluída**. Valores **negativos** (pagamentos/estornos) também são tratados de forma global no `InvoiceProcessingService` (issue #72): independentemente do parser, lançamentos com valor não positivo são **ignorados** ao gerar transações (ver "Geração de transações a partir da fatura").
...
- **Lançamentos com valor negativo (issue #72):** faturas reais (Nubank e Itaú) quase sempre trazem ao menos uma linha de **pagamento da fatura anterior** ou **estorno/crédito** com valor negativo. A regra de produto, aplicada de forma **global no `InvoiceProcessingService`** (idêntica para todas as origens), é **ignorar** essas linhas: lançamentos com valor não positivo (`valor <= 0`) **não geram transação** e não impedem o processamento. Isso mantém os relatórios como somatório dos gastos do período (estornos não reduzem artificialmente o total) e respeita o invariante da coluna `valor` (`@Positive` / `CHECK (valor > 0)`), que não aceita valores negativos. Uma fatura cujas linhas sejam todas não positivas é processada com sucesso, apenas sem transações.
- Os lançamentos **mantidos** (positivos) ainda são validados: `descricao` não vazia com no máximo 500 caracteres (alinhado com `VARCHAR(500)`). Uma descrição inválida faz o processamento falhar de forma controlada (`status = ERRO`) em vez de estourar uma constraint no commit.
...
A issue #29 adicionou o plugin **JaCoCo** ao `pom.xml`: o agente é preparado na fase `initialize`, o relatório é gerado em `target/site/jacoco/` na fase `test`, e a fase `verify` aplica um gate de cobertura — **mínimo de 70% de instruções e 70% de branches no pacote `com.goodfunds.service`** (atualmente acima de 90% e 85%, respectivamente). Cada service em `src/main/java/com/goodfunds/service/` tem uma classe de teste correspondente em `src/test/java/com/goodfunds/service/` (`AuthServiceTest`, `BudgetServiceTest`, `CategoryServiceTest`, `CategorySuggestionServiceTest`, `EstimateServiceTest`, `InvoiceProcessingServiceTest`, `InvoiceServiceTest`, `ReportCacheServiceTest`, `ReportServiceTest`, `TransactionServiceTest`). Pelo menos um controller tem teste de integração `MockMvc` — na verdade todos os controllers REST cobertos: `AuthControllerIntegrationTest`, `CategoryControllerIntegrationTest`, `TransactionControllerIntegrationTest`, `BudgetControllerIntegrationTest`, `InvoiceControllerIntegrationTest`, etc.
