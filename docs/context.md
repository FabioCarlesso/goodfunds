...
- Integrar o parser Nubank ao fluxo de upload (atualizar `Invoice` com `mesReferencia`/`totalValor`/`status=PROCESSADA` e gerar `Transaction`s), implementar listagem/detalhe de Invoices, parsers de ITAU/OUTROS, alem do relatorio restante (`/reports/summary`, com cache Caffeine) — a engine de estimativas (`GET /reports/estimate`, issue #20) e os relatorios `GET /reports/by-category` e `GET /reports/evolution` (issue #21) ja existem —, sobre a autenticacao existente.
