...
- Relatorios de leitura implementados (issue #21) em `ReportService`, escopados pelo usuario do JWT: `GET /reports/by-category?ref=YYYY-MM` retorna `[{categoryId, nome, tipo, total}]` com a soma das transacoes por categoria no mes de referencia, ordenada por `nome` (case-insensitive); `GET /reports/evolution?from=YYYY-MM&to=YYYY-MM` retorna a serie mensal de `receitas`/`despesas` para o intervalo, com todos os meses incluidos (zeros onde nao ha transacoes). A evolucao usa uma unica query agregada por mes e tipo de categoria (`sumByMonthAndTipo`). Validacoes: `from > to` e amplitude maior que 36 meses retornam 400 `invalid-filter`. Arredondamento monetario (2 casas, HALF_UP) centralizado em `util.MoneyUtils`, reutilizado por `EstimateService` e `ReportService`.
- Relatorio de resumo mensal implementado (issue #22): `GET /reports/summary?ref=YYYY-MM` retorna `{ ref, receitas, despesas, orcado, saldo, percentualOrcadoUsado }`. O parametro `ref` e opcional — omitido, usa o mes corrente via bean `Clock`. `percentualOrcadoUsado` e zero quando nao ha orcamento e pode exceder 100 quando as despesas superam o limite (comportamento intencional para sinalizar estouro).
- Cache Caffeine nos relatorios implementado (issue #23): `@Cacheable` em `ReportService` (`summary`/`byCategory`/`evolution`) e `EstimateService` (`estimate`), configurado em `CacheConfig` (`@EnableCaching` + `CaffeineCacheManager`, quatro caches, `expireAfterWrite=10min`, `maximumSize=1000`; interceptor de cache ordenado antes do de transacao para que um cache hit nao abra transacao read-only). Chaves prefixadas por `userId` (`"<userId>::..."`). Invalidacao por usuario via `ReportCacheService.evictUser(userId)`, acionada ao criar/editar/remover `Transaction`, criar/editar `Budget`, editar/remover `Category` (renomear ou trocar o tipo altera os relatorios) e ao processar fatura; quando ha transacao ativa a remocao ocorre apos o commit (evita repovoar o cache com dados pre-commit). Cobertura em `ReportCacheIntegrationTest` (cache ativo, invalidacao via API criando transacao e editando o tipo de categoria, escopo por usuario).
- Documentacao interativa da API implementada (issue #27): Springdoc/OpenAPI 3 (`springdoc-openapi-starter-webmvc-ui` 2.6.0). Swagger UI em `http://localhost:8080/swagger-ui.html` e spec em `/v3/api-docs`, liberados no `SecurityConfig`. Controllers anotados com `@Tag`/`@Operation`; esquema de seguranca `bearerAuth` (HTTP Bearer/JWT) declarado em `config/OpenApiConfig` (habilita o botao Authorize no Swagger UI) e referenciado pelos controllers protegidos via `@SecurityRequirement(name = OpenApiConfig.BEARER_SCHEME_NAME)`, enquanto `/auth/**` permanece publico. Em producao a spec e a UI sao desativadas (`springdoc.api-docs.enabled=false`, `springdoc.swagger-ui.enabled=false`) para nao expor a superficie da API. Cobertura em `OpenApiDocsIntegrationTest` (spec publica, esquema bearer, endpoint protegido vs `/auth/login` publico, Swagger UI acessivel) e `OpenApiDocsDisabledIntegrationTest` (spec/UI retornam 404 quando desativadas).
- Testes de backend cobrem schema/migrations, mapeamentos JPA, geracao/validacao de JWT, fluxos HTTP de autenticacao, testes unitarios do seed de categorias padrao em `AuthService`, testes unitarios do `TransactionService`, integracao HTTP completa do CRUD de Transactions, CRUD de Categories, CRUD de Budgets (`BudgetServiceTest`/`BudgetControllerIntegrationTest`), upload de Invoices, parsers de fatura PDF em `NubankInvoiceParserTest`/`InvoiceParserFactoryTest`, a engine de estimativas em `EstimateServiceTest` (unit, relogio fixo) e `ReportEstimateControllerIntegrationTest` (HTTP, isolamento por usuario e 401), os relatorios da issue #21 em `ReportServiceTest` (unit: ordenacao, zero-fill, `from > to`, teto de 36 meses) e `ReportByCategoryControllerIntegrationTest`/`ReportEvolutionControllerIntegrationTest` (HTTP, isolamento por usuario, 400 e 401), e o resumo mensal da issue #22 em `ReportServiceTest` (5 cenarios unit: campos corretos, sem dados, orcado zero, saldo negativo, `ref=null`) e `ReportSummaryControllerIntegrationTest` (7 cenarios HTTP: totais, `ref` padrao, sem dados, orcado zero, isolamento por usuario, formato invalido e 401).
...
- Integrar o parser Nubank ao fluxo de upload (atualizar `Invoice` com `mesReferencia`/`totalValor`/`status=PROCESSADA` e gerar `Transaction`s), implementar listagem/detalhe de Invoices e parsers de ITAU/OUTROS — a engine de estimativas (`GET /reports/estimate`, issue #20) e os relatorios `GET /reports/by-category`, `GET /reports/evolution` (issue #21) e `GET /reports/summary` (issue #22) ja existem —, sobre a autenticacao existente.
