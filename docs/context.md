...
- Integrar o parser Nubank ao fluxo de upload (atualizar `Invoice` com `mesReferencia`/`totalValor`/`status=PROCESSADA` e gerar `Transaction`s), implementar listagem/detalhe de Invoices, parsers de ITAU/OUTROS, alem de Reports, sobre a autenticacao existente.
