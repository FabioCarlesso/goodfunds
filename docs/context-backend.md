...
- A geracao de `Transaction`s a partir da fatura ja existe (`InvoiceProcessingService`, issue #16). O CRUD de `Budget` (`GET /budgets?ref=YYYY-MM`, `POST`, `PUT`, com unique `(user, category, mes, ano)` retornando 409 `budget-already-exists`) ja esta implementado (issue #19). A engine de estimativas e o endpoint `GET /reports/estimate` ja estao implementados (issue #20). Falta expor o gatilho de parse via endpoint REST, adicionar listagem/detalhe de Invoices e os demais relatorios (`/reports/summary`, `/reports/by-category`, `/reports/evolution`) com cache Caffeine, sobre a infraestrutura de seguranca ja existente.
