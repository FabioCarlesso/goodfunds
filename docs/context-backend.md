...
- Processamento de fatura (issue #16): `InvoiceProcessingService.process(invoiceId)` em `com.goodfunds.service` orquestra parse -> geracao de `Transaction`s. Carrega o PDF de `{app.uploads.dir}/{arquivo}`, usa o `InvoiceParserFactory` e cria uma `Transaction` por lancamento com `invoice=<id>`, `formaPagamento=CARTAO_CREDITO`, `user` da fatura e categoria default `Outros` (resolvida via `CategoryRepository.findFirstByUserIdAndNomeIgnoreCase`). Sucesso preenche `mesReferencia`/`totalValor` e marca `status=PROCESSADA`; falhas sao logadas e marcam `status=ERRO` sem propagar rollback. Idempotente: fatura ja `PROCESSADA` retorna sem reprocessar e um reprocessamento explicito remove lancamentos anteriores (`TransactionRepository.deleteByInvoiceId`) antes de recriar. Fatura inexistente -> `InvoiceNotFoundException` (404 `invoice-not-found`). Cobertura: `InvoiceProcessingServiceTest` (mocks) e `InvoiceProcessingServiceIntegrationTest` (fixture real, idempotencia e caminho de erro).
