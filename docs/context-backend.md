...
- Parsers de fatura PDF (issue #15) em `com.goodfunds.invoice.parser`: interface `InvoiceParser` (`OrigemFatura origem()`, `ParsedInvoice parse(File pdf)`), DTOs imutaveis `ParsedInvoice`/`ParsedInvoiceTransaction`, excecao `InvoiceParseException` e `InvoiceParserFactory` (Spring `@Component`) que recebe todas as implementacoes por DI e expoe `forInvoice(Invoice)`/`forOrigem(OrigemFatura)`. Implementacao MVP: `NubankInvoiceParser` usa Apache PDFBox 3.0.3 (`Loader.loadPDF` + `PDFTextStripper`) para extrair mes de referencia (`Mês de referência: <MES> de <ANO>`), total (`Valor total: R$ ...`) e lancamentos (`DD MMM Descricao R$ valor`); meses posteriores ao mes de referencia sao recuados para o ano anterior. `ItauInvoiceParser` (issue #69) cobre `OrigemFatura.ITAU` com a mesma stack, mas formato numerico do Itau: mes de referencia (`Mês de referência: MM/AAAA`), total (`Total desta fatura R$ ...`) e lancamentos (`DD/MM Descricao valor`, sem `R$` por linha), com a mesma regra de recuo de ano. Fixtures commitadas em `backend/src/test/resources/invoices/{nubank,itau}/sample-fatura.pdf`, regeraveis via `NubankInvoiceFixtures`/`ItauInvoiceFixtures` (test sources). `OUTROS` ainda nao tem parser. Cobertura: `NubankInvoiceParserTest`, `ItauInvoiceParserTest`, `InvoiceParserFactoryTest`.
...
- Cache Caffeine nos relatorios (issue #23): `CacheConfig` (`@EnableCaching` + `CaffeineCacheManager`) define quatro caches — `reportsSummary`, `reportsByCategory`, `reportsEvolution`, `reportsEstimate` — com `expireAfterWrite=10min` e `maximumSize=1000`. O interceptor de cache e ordenado antes do de transacao (`@EnableCaching(order = LOWEST_PRECEDENCE - 1)`), de modo que um cache hit retorna sem abrir a transacao read-only nem tomar conexao do pool. `@Cacheable` em `ReportService.summary/byCategory/evolution` e `EstimateService.estimate`, com chave prefixada por usuario (`"<userId>::..."`). Invalidacao por usuario via `ReportCacheService.evictUser(userId)`, que varre as chaves do cache nativo Caffeine pelo prefixo `"<userId>::"` (nao descarta o cache de outros usuarios); acionada nas escritas que afetam agregados: `TransactionService` (create/update/updateCategory/delete), `BudgetService` (create/update), `CategoryService` (update/delete — renomear ou trocar o tipo da categoria muda summary/by-category/estimate; create nao invalida porque categoria nova nao tem lancamentos) e `InvoiceProcessingService.process`. Quando ha transacao ativa a remocao e adiada para `afterCommit` (evita que uma leitura concorrente repovoe o cache com dados ainda nao commitados); sem transacao ativa, e imediata. `expireAfterWrite` funciona como rede de seguranca para entradas dependentes do mes corrente (`summary` sem `ref`, `estimate`) apos a virada do mes. Cobertura: `ReportCacheIntegrationTest` (cache ativo: escrita direta no banco nao aparece ate invalidar; invalidacao via `POST /transactions` e `PUT /categories/{id}` trocando o tipo; escopo por usuario).
- Documentacao OpenAPI/Swagger (issue #27): `config/OpenApiConfig` define os metadados da spec (`Goodfunds API`, v1) e o esquema de seguranca `bearerAuth` (HTTP Bearer/JWT), exposto pela constante reutilizavel `OpenApiConfig.BEARER_SCHEME_NAME`. Os controllers usam `@Tag`/`@Operation` para descricao e `@SecurityRequirement(name = OpenApiConfig.BEARER_SCHEME_NAME)` nos protegidos; `/auth/**` fica sem o requisito (publico). Swagger UI (`/swagger-ui.html`) e spec (`/v3/api-docs`) liberados no `SecurityConfig` e habilitados em dev; em prod desativados via `springdoc.api-docs.enabled=false` e `springdoc.swagger-ui.enabled=false`. Cobertura: `OpenApiDocsIntegrationTest` (spec/esquema/Swagger UI) e `OpenApiDocsDisabledIntegrationTest` (404 quando desativados).
...
A partir da issue #29 o build aplica o plugin JaCoCo: `./mvnw test` gera o relatorio em `target/site/jacoco/` e `./mvnw verify` aplica o gate de **>= 70% de cobertura por instrucao e por branch no pacote `com.goodfunds.service`** (atualmente acima de 90% e 85%, respectivamente). Falhar essa regra quebra o build.
...
- PDFBox e Springdoc ja estao no classpath porque fazem parte das dependencias base do MVP. Upload de PDF e parsers Nubank e Itau ja implementados; a origem `OUTROS` ainda nao tem parser registrado e fara `forOrigem` falhar com `InvoiceParseException` ate uma issue futura adicionar a implementacao.
- JWT sem refresh token no MVP (decisao explicita em `goodfunds-planejamento.md`).

## Proximos passos

- A geracao de `Transaction`s a partir da fatura ja existe (`InvoiceProcessingService`, issue #16). O CRUD de `Budget` ja esta implementado (issue #19). A engine de estimativas (`GET /reports/estimate`, issue #20) e os relatorios `GET /reports/by-category`, `GET /reports/evolution` (issue #21) e `GET /reports/summary` (issue #22) ja estao implementados. Falta o parser da origem `OUTROS`, sobre a infraestrutura de seguranca ja existente.
