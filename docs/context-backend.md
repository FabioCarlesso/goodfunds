...
- A geracao de `Transaction`s a partir da fatura ja existe (`InvoiceProcessingService`, issue #16). O CRUD de `Budget` (`GET /budgets?ref=YYYY-MM`, `POST`, `PUT`, com unique `(user, category, mes, ano)` retornando 409 `budget-already-exists`) ja esta implementado (issue #19). Falta expor o gatilho de parse via endpoint REST e adicionar listagem/detalhe de Invoices, alem de Reports, sobre a infraestrutura de seguranca ja existente.
