...
- Adicionar listagem/detalhe de Invoices integrando o parser ao fluxo (atualizar `Invoice` com `mesReferencia`/`totalValor`/`status=PROCESSADA` e gerar `Transaction`s), alem de Budgets e Reports, sobre a infraestrutura de seguranca ja existente.
